...
static char** create_new_map(t_main *cub)
{
	...
	with_map = cub->map.ncols + 4;
    new_map	= malloc((cub->map.nrows) * sizeof(char *));
    if (!new_map)
	{
	...
	count = -1;
	while (++count < cub->map.nrows)
	{
		new_map[count] = creat_line_with_empty(with_map);
...
static int check_char(char **map, int row, int colum, int last_row)
{
	...
	if ((last_row - 1) > row)
	{
		...
		if ((len > colum) && !ft_isspace(map[row + 1][colum]) && map[row + 1][colum] != '1')
			return (1);
	}
	if (row > 0)
	{
